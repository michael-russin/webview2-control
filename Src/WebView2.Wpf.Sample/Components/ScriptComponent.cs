...
namespace MtrDev.WebView2.WinForms.Sample.Components
{
    public class ScriptComponent :ComponentBase
    {
        ...
        {
            if (command == ScriptCommands.InjectScript)
            {
                InjectScript();
            }
            else if (command == ScriptCommands.AddInitializeScript)
            {
                AddInitializeScript();
            }
            else if (command == ScriptCommands.RemoveInitializeScript)
            {
                RemoveInitializeScript();
            }
            else if (command == ScriptCommands.PostMessageString)
            {
                SendStringWebMessage();
            }
            else if (command == ScriptCommands.PostMessageJSON)
            {
                SendJsonWebMessage();
            }
            else if (command == ScriptCommands.SubscribetoCDPevent)
            {
                SubscribeToCdpEvent();
            }
            else if (command == ScriptCommands.CallCDPmethod)
            {
                CallCdpMethod();
            }
            else if (command == ScriptCommands.AddCOMobject)
            {
                AddComObject();
            }
            else if (command == ScriptCommands.OpenDevToolsWindow)
            {
                OpenDevToolsWindow();
            }
        }
        ...
        public void InjectScript()
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                _webView2.ExecuteScript(dialog.Input, (args) =>
                {
                    ...
                    MessageBox.Show(args.ResultAsJson, "ExecuteScript Result", MessageBoxButton.OK);
                });
            }
        }
