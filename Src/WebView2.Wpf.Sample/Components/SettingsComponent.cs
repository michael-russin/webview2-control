...
namespace MtrDev.WebView2.WinForms.Sample.Components
{
    public class SettingsComponent : ComponentBase
    {
        ...
        {
            if (command == SettingsCommands.BlockedDomains)
            {
                ChangeBlockedSites();
            }
            else if (command == SettingsCommands.SetUserAgent)
            {
                ChangeUserAgent();
            }
            else if (command == SettingsCommands.ToggleJavaScript)
            {
                ToggleJavascript();
            }
            else if (command == SettingsCommands.ToggleWebMessaging)
            {
                ToggleWebMessages();
            }
            else if (command == SettingsCommands.ToggleFullscreenallowed)
            {
                ToggleFullScreenAllowed();
            }
            else if (command == SettingsCommands.ToggleStatusBarenabled)
            {
                ToggleStatusBar();
            }
            else if (command == SettingsCommands.ToggleDevToolsenabled)
            {
                ToggleDevToolsEnabled();
            }
            else if (command == SettingsCommands.ToggleBlockimages)
            {
                ToggleBlockedImages();
            }
            else if (command == SettingsCommands.UseDefaultScriptDialogs)
            {
                UseDefaultScriptDialogs();
            }
            else if (command == SettingsCommands.UseCustomScriptDialogs)
            {
                UseCustomScriptDialogs();
            }
            else if (command == SettingsCommands.UseDeferredScriptDialogs)
            {
                UseDeferredScriptDialogs();
            }
            else if (command == SettingsCommands.CompleteDeferredScriptDialog)
            {
                CompleteScriptDialogDeferral();
            }
            else if (command == SettingsCommands.ToggleContextMenusEnabled)
            {
                ToggleContextMenuEnagled();
            }
            else if (command == SettingsCommands.ToggleRemoteObjectsAllowed)
            {
                ToggleRemoteObjectsAllowed();
            }
            else if (command == SettingsCommands.ToggleZoomControlAllowed)
            {
                ToggleZoomControlAllowed();
            }
        }
        ...
        private void WebView2PermissionRequested(object sender, PermissionRequestedEventArgs e)
        {
            ...
            MessageBoxResult response = MessageBox.Show(message, "Permission Request", MessageBoxButton.YesNoCancel, MessageBoxImage.Warning);

            WEBVIEW2_PERMISSION_STATE state =
                  response == MessageBoxResult.Yes ? WEBVIEW2_PERMISSION_STATE.WEBVIEW2_PERMISSION_STATE_ALLOW
                : response == MessageBoxResult.No ? WEBVIEW2_PERMISSION_STATE.WEBVIEW2_PERMISSION_STATE_DENY
        ...
        public void ToggleJavascript()
        {
            ...
            MessageBox.Show("JavaScript will be "
                           ...
                           "Settings change", MessageBoxButton.OK);
        }
        ...
        public void ToggleRemoteObjectsAllowed()
        {
        ...
        public void ToggleZoomControlAllowed()
        {
